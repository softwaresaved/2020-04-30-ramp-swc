...
try:
    ...
    print("[Failed] The 'python' command on this system points to an old version of python.")
    print("Please try again using 'python3'")
    sys.exit(1)
...
__version__ = '0.1'
# URL for results upload

target_uri = "http://othmar.mooo.com:6868/submit"
    ...
    if failures:
        ...
        for failure in failures:
            if failure not in printed:
                ...
                fails.append(failure.checker.long_name)
        return False
    return True
...
if __name__ == '__main__':
    ...
    if not passed:
        print()
        print("The software indicated above is NOT installed correctly.")
        print("Please check that you have carefully followed the instructions provided.")
        print("For assistance, please send an email: rsg-info@soton.ac.uk")
        print("Please include ALL of the output from this script, up to this point.")
        if options.verbose:
            print()
            print_system_info()
    ...
    if passed:
        ...
        status = 'failed'
    data = {'mac': mac, 'platform': platform, 'status': status, 'failures': fails}
    ...
    try:
        ...
        print("Unable to log test results.")
